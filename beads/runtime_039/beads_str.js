...
export function conv_num_to_hex(id) {
    ...
    for (std.bytes_put_num(buf, id), buf.position = 0, pos = 0; pos < 8; pos++) ss += digit = conv_hex_byte(code = std.bytes_get_byte(buf));
    return ss;
};
...
export function conv_unicode_to_bytes(ss) {
    let code, i, len, buf = std.new_bytes(10, std.BIG_ENDIAN), result = "";
    for (std.bytes_put_str(buf, ss), len = buf.position - 2, buf.position = 2, i = 0; i < len; i++) result += "%" + conv_hex_byte(code = std.bytes_get_byte(buf));
    return ss;
};
