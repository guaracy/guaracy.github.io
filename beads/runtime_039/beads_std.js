...
g_enum_ss[9007191781826255] = "BIG_ENDIAN";
...
g_enum_ss[9007190985638604] = "app_hash";

export const F_app_name = 9007191050153831;

g_enum_ss[9007191050153831] = "app_name";
...
g_enum_ss[9007193920084361] = "date_weekday";
...
g_enum_ss[9007191616321804] = "debug_date";

export const F_debug_kind = 9007193734782890;

g_enum_ss[9007193734782890] = "debug_kind";

export const F_debug_note = 9007190554134004;

g_enum_ss[9007190554134004] = "debug_note";

export const F_dpi = 9007193660320951;

g_enum_ss[9007193660320951] = "dpi";

export const F_env_version = 9007192684405836;

g_enum_ss[9007192684405836] = "env_version";
...
g_enum_ss[9007194068951371] = "LITTLE_ENDIAN";
...
g_enum_ss[9007192502734777] = "meter";
...
export const V_debug_runtime = 9007190789640092;

g_enum_ss[9007190789640092] = "debug_runtime";
...
g_enum_ss[9007192212261205] = "LOCALIZED";
...
g_enum_ss[9007190655482165] = "PLACEHOLDER";
...
g_enum_ss[9007192707741773] = "Volume", FIELDS[F_env_version] = !0, FIELDS[F_app_version] = !0, FIELDS[F_args] = !0, FIELDS[F_base_to_unit] = !0, FIELDS[F_bchildren] = !0, FIELDS[F_bdepth] = !0, FIELDS[F_bexpanded] = !0, 
...
function js_set_screen_size() {
    if (IS_NODE) SCREEN_H = 1024, SCREEN_V = 768, SCREEN_DPI = 96; else {
        ...
        SCREEN_H = s.width, SCREEN_V = s.height, SCREEN_DPI = 96 * dev_ratio;
        const list = [ {
            hh: 320,
            vv: 480,
            ratio: 2,
        ...
        for (var item of list) if ((SCREEN_H == item.hh && SCREEN_V == item.vv || SCREEN_H == item.hh && SCREEN_V == item.vv) && item.ratio - dev_ratio < .01) {
            SCREEN_DPI = item.actual / dev_ratio;
            break;
        }
    }
...
function bytes_put_relpath(buf, argpath, basis) {
    ...
    for (TRACE_ENCODE && logg(`bytes_put_relpath, offset=${buf.position}, nlevels=${nlevels}`), bytes_put_byte(buf, NOV_RELPATH), bytes_put_byte(buf, nlevels), i = 0; i < nlevels; i++) TRACE_ENCODE && logg(`  relpath level=${i}, offset=${buf.position}`), 
    bytes_put_val(buf, argpath.key[basisn + i], 0);
}

function bytes_get_relpath(buf, basis) {
    var i, subscript, net = basis.clone(), nlevels = bytes_get_byte(buf);
...
export function bytes_put_val(buf, val, flag) {
    if ("number" == typeof val) 0 == val ? (bytes_put_byte(buf, flag | NOV_0), TRACE_ENCODE && logg(str.conv(" put_val, offset={}, ZERO", buf.position))) : val == Y ? (bytes_put_byte(buf, flag | NOV_Y), TRACE_ENCODE && logg(str.conv(" put_val, offset={}, Y", buf.position))) : val == N ? (bytes_put_byte(buf, flag | NOV_N), 
    TRACE_ENCODE && logg(str.conv(" put_val, offset={}, N", buf.position))) : val == U ? (bytes_put_byte(buf, flag | NOV_U), TRACE_ENCODE && logg(str.conv(" put_val, offset={}, U", buf.position))) : Number.isNaN(val) ? (bytes_put_byte(buf, flag | NOV_ERR), 
    TRACE_ENCODE && logg(str.conv(" put_val, offset={}, ERR", buf.position))) : (bytes_put_byte(buf, flag | NOV_NUM), TRACE_ENCODE && logg(str.conv(" put_val, offset={}, num={}", buf.position, val)), bytes_put_num(buf, val)); else if ("string" == typeof val) bytes_put_byte(buf, flag | NOV_STR), 
    TRACE_ENCODE && logg(str.conv(" put_val, offset={}, str={}", buf.position, val)), bytes_put_str(buf, val); else if (val instanceof a_path) bytes_put_byte(buf, flag | NOV_PATH), TRACE_ENCODE && logg(str.conv(" put_val, offset={}, path", buf.position)), 
    bytes_put_path(buf, val); else if (val instanceof a_function) {
        ...
        if (!(val instanceof a_meas)) throw val instanceof a_image ? (bytes_put_byte(buf, flag | NOV_IMAGE), TRACE_ENCODE && logg(str.conv(" put_val, offset={}, image", buf.position)), new Error("a_image not yet")) : val instanceof a_sound ? (bytes_put_byte(buf, flag | NOV_SOUND), 
        TRACE_ENCODE && logg(str.conv(" put_val, offset={}, sound", buf.position)), new Error("a_sound not yet")) : (bytes_put_byte(buf, flag | NOV_OBJECT), new Error(`unsupported type ${typeof val}`));
...
export function bytes_get_val(buf) {
    return bytes_get_val2(buf, bytes_get_byte(buf));
};

function bytes_get_val2(buf, valkind) {
    let val_meas, val_hash, val_func;
    switch (TRACE_DECODE && logg(str.conv("get_val at {}, valkind={}", buf.position - 1, valkind)), valkind) {
      case NOV_NUM:
...
export function decode_tree_bin(buf, destp) {
    ...
    opcode) {
      case SUB_NUM:
        sub_num = bytes_get_num(buf), dest.key.push(sub_num), TRACE_DECODE && logg(str.conv("  push subscript {}", sub_num));
        break;

      case SUB_STR:
        sub_str = bytes_get_str(buf), dest.key.push(sub_str), TRACE_DECODE && logg(str.conv("  push str subscript {}", sub_str));
        break;
