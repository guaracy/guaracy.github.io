...
export function rtl_init() {
    ...
    ERR_TREE = new a_tree(_M, "$ERR_TREE", 0), std_add_meta(), str.str_add_meta(), loom_history = new a_tree(_M, "$loom_history", NF_TOPLEVEL | NF_STATEFUL), runtime = new a_tree(_M, "$runtime", NF_TOPLEVEL | NF_STATEFUL), 
    blackbox_runtime = new a_tree(_M, "$blackbox_runtime", NF_TOPLEVEL | NF_STATEFUL), js_set_screen_size(), setv(_M, 0, runtime, F_app_version, "1");
    ...
    setv(_M, 0, runtime, F_major_stepx, 2), setv(_M, 0, runtime, F_touch_kind, TOUCH_NONE), !IS_NODE) {
        window.matchMedia("(pointer: coarse)").matches && setv(_M, 0, runtime, F_touch_kind, TOUCH_FINGER), k_init(), window.addEventListener("keydown", js_keydown), window.addEventListener("mousedown", js_mousedown), 
        window.addEventListener("mouseup", js_mouseup), window.addEventListener("mousemove", js_mousemove), window.addEventListener("touchstart", js_touchstart), window.addEventListener("touchmove", js_touchmove), 
        window.addEventListener("touchend", js_touchend), window.addEventListener("touchcancel", js_touchcancel), window.oncontextmenu = (e => (e.preventDefault(), !1)), window.addEventListener("resize", js_resize);
...
export function div_add(parent, arg_id, arg_flags, arg_meas, arg_unit, arg_func, arg_order = 0, arg_condition = Y) {
    ...
    0 == (arg_flags & DIVF_VERT) ? (parent.div.horz_slices.push(new_slice), parent.div.horz_slices.length > 500 && internal_err("More than 500 columns; probable infinite loop in " + parent.blabel)) : (parent.div.vert_slices.push(new_slice), 
    parent.div.vert_slices.length > 1e3 && internal_err("More than 1000 rows; probable infinite loop in " + parent.blabel));
};
