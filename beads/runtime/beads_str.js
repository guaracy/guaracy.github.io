      ...
      case 1:
        if ("]" == cc) {
            switch (token += "]") {
              ...
              case "[year2]":
                token = (nn = dateobj.fullYear) >= 0 && nn <= 9999 ? to_str(nn % 100) : std.ERR_STR;
                break;
              ...
              default:
                std.CHECKS ? std.argument_err("### internal error, unknown conversion: " + token) : token = std.ERR_STR;
            }
...
export function enum_to_str(val, use_stringtable = !1) {
    let ss;
    return std.CHECKS && !is_enumb(val) && std.halt(), ss = void 0 === std.g_enum_ss[val] ? "ENUM:" + to_str(val -= std.FIRST_ENUM, {
        min: 10,
        zero_pad: std.Y
    }) : std.g_enum_ss[val];
};
...
export function to_str(val, options = null) {
    let nshort, old_digits, ss, base = 10, currency_cc = "$ ", decimal_cc = ".", digits = std.U, is_autodec = !1, is_autoinc = !1, is_currency = !1, is_human = !1, is_kflag = !1, is_negparen = !1, is_percent = !1, is_posplus = !1, is_showu = !1, is_thou = !1, is_zerodrop = !0, is_zeropad = !1, language = "ENG", maxlen = 999, minlen = 0, suffix = null, thou_cc = ",", units = std.U;
    ...
    } else if (val instanceof File) ss = "<FILE:" + val.name + ">"; else if (val === std.INFINITY) ss = "INFINITY", is_zeropad = !1; else if (val === std.NEG_INFINITY) ss = "-INFINITY", is_zeropad = !1; else if (val === std.Y) ss = "Y", 
    is_zeropad = !1; else if (val === std.N) ss = "N", is_zeropad = !1; else if (is_enumb(val)) ss = enum_to_str(val), is_zeropad = !1; else if (Number.isNaN(val)) ss = "ERR", is_zeropad = !1; else {
...
export function tree_to_str(base, options = null) {
    ...
    if (base instanceof std.a_tree) base_path = std.addr(base); else if (base instanceof std.a_path) {
        if (null === base) return null;
        base_path = base;
    } else std.argument_err("expected path or tree");
    for (let property in options) switch (property) {
...
export function str_pos(haystack, needle, options) {
    var start = 0, ignore_case = !1, list = new a_tree("str", "str_pos");
    ...
    return str_find(_M, 0, haystack, needle, std.addr(list), {
        start: start,
...
export function str_inject(input, ...parms) {
    ...
    for (let parm in parms) pattern = "$" + String(1), result = result.replaceAll(pattern, parm);
    return result;
};
...
export function split_lines(module, loc, buf, dest) {
    ...
    startpos = i; ;) {
        ...
        code = buf.charCodeAt(i), i += 1, code === CODE_CR ? (g_delimiter = DELIM_CR, emit_fragment(startpos, i - 1), i < len && buf.charCodeAt(i) === CODE_NL && (i += 1, g_delimiter = DELIM_CRLF), startpos = i) : code === CODE_NL && (emit_fragment(startpos, i - 1), 
        startpos = i, g_delimiter = DELIM_LF);
    }
};
        ...
        if ("}" === cc) {
            switch (token += "}") {
              case "{}":
                ...
                } else parm instanceof RegExp ? token = parm.toString() : std.argument_err("bad type");
                break;
...
export function conv_num_to_hex(id) {
    let digit, pos, code, ss = "", buf = std.new_bytes(8, std.BIG_END);
    for (std.bytes_put_num(buf, id), buf.position = 0, pos = 0; pos < 8; pos++) ss += digit = conv_hex_byte(code = std.bytes_get_card8(buf));
    return ss;
};
...
export function conv_unicode_to_bytes(ss) {
    let code, i, len, buf = std.new_bytes(10, std.BIG_END), result = "";
    for (std.bytes_put_str(buf, ss), len = buf.position - 2, buf.position = 2, i = 0; i < len; i++) result += "%" + conv_hex_byte(code = std.bytes_get_card8(buf));
    return ss;
};
