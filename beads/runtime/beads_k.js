...
export function k_layer(parent, drawf, options) {
    let in_b, in_l, in_r, in_t, indent, newb, canvas_dest = null, dpi = std.U, height = parent.bounds.height, left = 0, opacity = 1, pin = std.U, skewx = 0, skewy = 0, top = 0, tx = 0, ty = 0, width = parent.bounds.width;
...
export function find_xy_in_grid(b, event, localx, localy) {
    ...
    return (logcol > div.ncols || logrow > div.nrows) && (in_dead_space = !0), in_dead_space ? (logcol = std.U, logrow = std.U, cumx = std.U, idx = std.U, idy = std.U) : cumx = div.grid_tblr ? (logcol - 1) * div.nrows + logrow : (logrow - 1) * div.ncols + logcol, 
    std.setv(M, 0, b.extra, std.F_cell_seq, cumx), std.setv(M, 0, b.extra, std.F_cell, std.F_x, logcol), std.setv(M, 0, b.extra, std.F_cell, std.F_y, logrow), std.setv(M, 0, b.extra, std.F_cell_id, std.F_x, idx), 
    std.setv(M, 0, b.extra, std.F_cell_id, std.F_y, idy), (absorbval = null != b.drawer.parms ? tracker.apply(null, [ b, event ].concat(b.drawer.parms)) : tracker(b, event)) == std.Y && (g_last_majorx > 0 && (std.setv(M, 0, std.runtime, std.F_major_steps, g_last_majorx, std.F_hist_cell_cum, cumx), 
